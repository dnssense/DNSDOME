...
export function mapBaseFilterRequestUiFieldsToServiceFields(
  filter: BaseFilterInput,
  mappingPairs: Map<string, string>
): BaseFilterInput {
  if (mappingPairs) {
...
export function mapBaseServiceModelToUIModel<T, K>(
  mapper: (arg: K) => T,
  json: BaseResponseServiceModel<K>
): BaseUIResponseModel<T> {
  ...
  if (json.results) {
    if (json.results instanceof Array) {
      ...
      if (list) uiModel.results = list
    } else {
      uiModel.results = mapper(json.results)
    }
  }
  if (json.total_count) uiModel.total_count = json.total_count
  return uiModel
}
...
function handleCustomFvFilter(i: FilterBadgeModelV2, serviceModel: BaseFilterRequestServiceModel) {
  i.values?.forEach((v: FilterBadgeValueV2) => {
    ...
    if (v.value === FirstlyVisitedByOption.Company) {
      filterItem = {
        name: TrafficColumnNames.FVCompany,
      } as BaseServiceFilterModel
    } else if (v.value === FirstlyVisitedByOption.Network) {
      filterItem = {
        name: TrafficColumnNames.FVNetwork,
      } as BaseServiceFilterModel
    } else if (v.value === FirstlyVisitedByOption.Host) {
      filterItem = {
        name: TrafficColumnNames.FVHost,
      } as BaseServiceFilterModel
    } else {
      return
    }
...
function mapUICriteriaToServiceModel(
  i,
  serviceModel: BaseFilterRequestServiceModel,
  ...
  i.values?.forEach((v: FilterBadgeValueV2) => {
    filterItem.value.push(
      mapper
        ? mapper(v.value !== null && v.value !== undefined ? v.value : 'N/A')
...
export function mapBaseUIFilterRequestToServiceModel(
  uiModel: BaseFilterUIRequestModel
): BaseFilterRequestServiceModel {
  ...
  if (uiModel.filter) {
    ...
    uiModel.filter?.filterList?.forEach((i: FilterBadgeModelV2) => {
      ...
      if (i.name === FilterConstant.IS_FV) {
