...
export class RoamingComponent implements OnInit, AfterViewInit {
  ...
  get isFormValid() {
    return (
      this.selectedClient.agentAlias.trim().length > 0 && this.selectedClient.rootProfile.id > 0
    )
  }
  ...
  @ViewChild('inputUninstallPassword') inputUninstallPassword: ElementRef
  @ViewChild('inputDisablePassword') inputDisablePassword: ElementRef
  ...
  @ViewChild('inputAgentUninstallPassword') inputAgentUninstallPassword: ElementRef
  @ViewChild('inputAgentDisablePassword') inputAgentDisablePassword: ElementRef
  ...
  columns: RkTableColumnModel[] = [
    { id: 1, name: 'agentAlias', displayText: this.translatorService.translate('DeviceName') },
    {
      id: 2,
      name: 'rootProfile',
      displayText: this.translatorService.translate('SecurityProfile'),
    },
    ...
    { id: 4, name: 'isAlive', displayText: this.translatorService.translate('Alive') },
    {
      id: 5,
      name: 'isUserDisabled',
      displayText: this.translatorService.translate('UserDisabled'),
    },
    { id: 6, name: 'isDisabled', displayText: this.translatorService.translate('Protection') },
    {
      id: 7,
      name: 'isUserDisabledSmartCache',
      displayText: this.translatorService.translate('UserDisabledSmartCache'),
    },
    {
      id: 8,
      name: 'isSmartCacheEnabled',
      displayText: this.translatorService.translate('SmartCache'),
    },
  ...
  tableHeight =
    window.innerWidth > 768
      ? window.innerHeight - 373 - (document.body.scrollHeight - document.body.clientHeight)
      : null
  selectAll: boolean
  ...
  ngOnInit(): void {
    ...
    this.categoryOptions.forEach(
      (opt) => (opt.displayText = this.translatorService.translate(opt.displayText))
    )
    this.boxService
      .getWindowsRoamingClientVersion()
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((version) => {
        this.roamingClientVersion = version
      })
    this.boxService
      .getMacRoamingClientVersion()
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((version) => {
  ...
  ngAfterViewInit() {
    this.configureModal.close.subscribe()
  ...
  defaultRoamingProfileChanged(event) {
  ...
  loadClients() {
    ...
    this.roamingService.getClients().subscribe((res) => {
      ...
      this.clients
        .filter((x) => x.agentGroup)
        .forEach((x) => {
          const item: RkSelectModel = {
            displayText: x.agentGroup.groupName,
            value: x.agentGroup.groupName,
            selected: false,
          }
      ...
      this.clientsForShow = this.isGroupedRadioButtonSelected
        ? this.clientsGroupedFiltered
        : this.clientsUngroupedFiltered
  ...
  checkIPNumberForDoNotTouchIps(event: KeyboardEvent, inputValue: string) {
    ...
    if (virtualBox?.conf) {
      try {
        ...
        if (boxConf.donttouchdomains) {
          this.dontDomains = boxConf.donttouchdomains
            .split(',')
            .filter(Boolean)
        ...
        this.fillSecurityProfilesSelectForDefaultSettings(
          this.securityProfiles,
          this.selectedDefaultRomainProfileId
        )
      } catch (ignore) {
        console.log(ignore)
      }
  ...
  getConfParameters() {
    return this.boxService.getVirtualBox().pipe(
      map((res) => {
        this.virtualBox = res
        this.fillBoxDefaultSettings(this.virtualBox)
      })
  ...
  searchByKeyword() {
    if (this.searchKey) {
      ...
      this.clientsGroupedFiltered = this.clients.filter((f) => {
        return (
          f.agentAlias.toLowerCase().includes(key) ||
  ...
  saveRoamingGlobalSettings() {
    ...
    const domains = this.dontDomains
      .map((domain) => (domain[0] !== '.' ? '.'.concat(domain) : domain))
      .join(',')
    ...
    this.boxService.saveBoxConfig(request).subscribe(() => {
  ...
  checkIsValidDomaind(d: string): string | null {
    return d.toLocaleLowerCase().replace('https://', '').replace('http://', '')
  }
  ...
  openChangeGroup() {
    this.selectedAgentsForChangeAddGroup = this.clients
      .filter((x) => x.selected)
  ...
  async updateAgents(type: 'cache' | 'protection', mod: 'enable' | 'disable') {
    ...
    const changedAgents = filteredClients.filter((agent) => {
      ...
      return (
        (agent.isSmartCacheEnabled && mod === 'disable') ||
        (!agent.isSmartCacheEnabled && mod === 'enable')
      )
    })
    for (const agent of changedAgents) {
      const conf: AgentConf = {
        ...
        isSmartCacheEnabled:
          type === 'cache' ? (mod === 'enable' ? 1 : 0) : agent.isSmartCacheEnabled ? 1 : 0,
  ...
  saveChangedGroup() {
    ...
    this.selectedAgentsForChangeAddGroup.forEach((x) => {
      ...
      const foundGroup = this.groupedClients.find(
        (group) => group.agentGroup.groupName === this.selectedGroupName
      )
  ...
  openCreateGroup() {
    ...
    this.noGroupedClients = this.clients
      .filter((x) => !x.agentGroup)
      .map((x) => JSON.parse(JSON.stringify(x)))
    this.groupMembers = []
    this.groupModal.toggle()
  ...
  saveOrUpdateGroup() {
    ...
    if (this.groupOperation == 'Create New') {
      ...
      if (
        ...
        this.selectedProfileRadio !== this.selectedProfileRadioBeforeEdit
      ) {
        // name changed
  ...
  openUpdateGroup(agents: Agent[]) {
    ...
    this.noGroupedClients = this.clients
      .filter((x) => !x.agentGroup)
      .map((x) => JSON.parse(JSON.stringify(x)))
    this.groupMembers = this.clients
      .filter((x) => x.agentGroup && x.agentGroup.groupName == this.groupName)
      .map((x) => JSON.parse(JSON.stringify(x)))
  ...
  deleteGroup(agents: Agent[]) {
    this.alertService
      .alertWarningAndCancel(
        `${this.staticMessageService.areYouSureMessage}?`,
        `${this.staticMessageService.groupWillBeDeletedMessage}`
      )
      .subscribe(() => {
  ...
  saveRoamingClient() {
    if (this.selectedClient && this.isFormValid) {
      const conf: AgentConf = {
        isDisabled: this.selectedClient.isDisabled ? 1 : 0,
        isSmartCacheEnabled: this.selectedClient.isSmartCacheEnabled ? 1 : 0,
        disablePassword: this.selectedClient.disablePassword,
        uninstallPassword: this.selectedClient.uninstallPassword,
      }
  ...
  deleteClient(id: number) {
    this.alertService
      ...
      .subscribe((res) => {
        if (res && id && id > 0) {
          this.roamingService.deleteClient(id).subscribe(() => {
            this.deletedAgentRoamingClientMessage()
            this.loadClients()
          })
        }
  ...
  saveAgentConf(agent: Agent) {
    const conf: AgentConf = {
      ...
      isSmartCacheEnabled: agent.isSmartCacheEnabled ? 1 : 0,
      disablePassword: agent.disablePassword,
      uninstallPassword: agent.uninstallPassword,
    }

    return this.agentService.saveAgentConf(agent.uuid, conf).subscribe(() => {
      this.notification.success(this.staticMessageService.savedAgentConfMessage)
    })
  }
  ...
  fillSecurityProfilesArray(agent?: Agent) {
    this.securityProfilesForSelect = this.securityProfiles.map((elem, index) => {
      ...
      if (this.saveMode === 'NewProfile') {
        ...
        if (
          this.selectedClient.rootProfile &&
          this.selectedClient.rootProfile.name &&
          this.selectedClient.rootProfile.id === elem.id
        ) {
          obj.selected = true
        }
      }
    ...
    this.clientsForShow = this.clientsForShow.sort((a, b) => {
      if (name === 'agentGroup') {
        ...
        if (a.isAlive === b.isAlive) {
          sortValue =
            moment(a.insertDate).toDate().getTime() - moment(b.insertDate).toDate().getTime()
        } else {
  ...
  exportAs(extention: ExportTypes) {
    const exportedTypeList = this.isGroupedRadioButtonSelected
      ? this.clientsGroupedFiltered
      : this.clientsUngroupedFiltered
    if (exportedTypeList && exportedTypeList?.length > 0) {
      const tableData = JSON.parse(JSON.stringify(exportedTypeList))

      tableData.forEach((data) => {
        data.alive = data.isAlive
          ? 'Alive'
          : 'Last Seen ' + (data?.insertDate ? data?.insertDate : 'More Than One Week')
        delete data.id
        delete data.userId
      ...
      if (extention === 'excel') {
        this.excelService.exportAsExcelFile(
          tableData,
          'ClientsReport-' + d.getDate() + '-' + d.getMonth() + '-' + d.getFullYear()
        )
      }
    }
  }
  ...
  exportGroupsAs(extention: ExportTypes) {
    if (this.groupedClients && this.groupedClients.length > 0) {
      const tableData = JSON.parse(JSON.stringify(this.groupedClients))
      tableData.forEach((data) => {
        data['Name'] = data.name
